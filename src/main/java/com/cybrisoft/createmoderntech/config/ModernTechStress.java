...
public class ModernTechStress extends CStress {
    ...
    public static <B extends Block, P> NonNullUnaryOperator<BlockBuilder<B, P>> setImpact(final double value) {
        return (builder) -> {
            assertFromModernTech(builder);
            ResourceLocation id = CreateModernTech.path(builder.getName());
    ...
    private static void assertFromModernTech(final BlockBuilder<?, ?> builder) {
        if (!builder.getOwner().getModid().equals(CreateModernTech.MODID)) {
            throw new IllegalStateException("Non-ModernTech blocks cannot be added to ModernTech's config.");
        }
    }
