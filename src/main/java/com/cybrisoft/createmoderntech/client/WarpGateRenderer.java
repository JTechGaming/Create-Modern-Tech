...
public class WarpGateRenderer {
    private static final List<WarpGateTerminalBlockEntity> gates = new CopyOnWriteArrayList<>();

    public static void register(WarpGateTerminalBlockEntity be) {
        if (!gates.contains(be)) gates.add(be);
    }

    public static void remove(WarpGateTerminalBlockEntity be) {
        gates.remove(be);
    }

    public static List<WarpGateTerminalBlockEntity> getGates() {
        return gates;
    }
    ...
    public static void onRender(RenderLevelStageEvent event) {
        ...
        for (WarpGateTerminalBlockEntity be : gates) {
            drawGate(be, ms, bufferSource, cam);
        }

        ms.popPose();
    }

    public static void drawGate(WarpGateTerminalBlockEntity be, PoseStack ms, MultiBufferSource.BufferSource bufferSource, Vec3 cam) {
        if (be.isRemoved()) {
            gates.remove(be);
            return;
        }
        ...
        if (cam.distanceToSqr(gateCenter) > renderDistance * renderDistance) return;

        ms.pushPose();
        BlockPos pos = be.getBlockPos();
        ms.translate(pos.getX(), pos.getY(), pos.getZ());
        ms.translate(be.ponderRenderOffset.x, be.ponderRenderOffset.y, be.ponderRenderOffset.z);
